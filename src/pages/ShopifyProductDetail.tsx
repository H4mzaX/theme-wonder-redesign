...
const ShopifyProductDetail = () => {
  ...
  // Bug 1 fix: use getLatestCheckoutUrl via ref so we get the URL
  // after addItem's Shopify sync completes, not stale state
              ...
              <div className="lg:hidden">
                <div
                  ...
                  onTouchEnd={(e) => {
                    ...
                    const diffY = el._startY - e.changedTouches[0].clientY;
                    ...
                    if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > threshold) {
                      if (diffX > 0 && activeGalleryImg < images.length - 1)
                  ...
                  {images.map((_, i) => (
                    <button
                      ...
                      className={`rounded-full transition-all duration-150 ${
                        i === activeGalleryImg ? "w-6 h-1.5 bg-foreground" : "w-1.5 h-1.5 bg-foreground/20"
                      }`}
              ...
              <div className="lg:sticky lg:top-[80px] lg:pb-10">
                  ...
                  {compareAtPrice > price && (
                    <>
                      <span className="text-base text-muted-foreground line-through">
                        MRP {currencySymbol}{compareAtPrice.toLocaleString("en-IN")}
                      </span>
                ...
                >
                  {highlightCards.map((card) => (
                    <div key={card.label} className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-muted/50 border border-border/40">
                      <div className="w-8 h-8 rounded-full bg-background border border-border flex items-center justify-center">
                        <card.Icon className="w-4 h-4 text-foreground" />
                      </div>
                      <span className="text-[10px] font-semibold text-foreground leading-none text-center">{card.label}</span>
                    </div>
                  ))}
                </motion.div>
                ...
                >
                  {offers.map((offer) => (
                    <div key={offer.label} className="flex items-center gap-2.5 flex-none px-3.5 py-2.5 rounded-xl border border-dashed border-border bg-muted/30">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-foreground leading-none">{offer.label}</p>
                        <p className="text-[10px] text-muted-foreground leading-none mt-0.5">{offer.desc}</p>
                      </div>
                    </div>
                  ))}
                </motion.div>
                ...
                >
                  ...
                  >
                    {isCartLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Add to Cart"}
                  </button>
                  <button
                ...
                >
                  ...
                  <div className="flex gap-1.5">
                    {["Visa", "MC", "UPI", "GPay"].map((m) => (
                      <span key={m} className="text-[9px] font-semibold bg-muted px-1.5 py-0.5 rounded">{m}</span>
                    ))}
                  </div>
                </motion.div>
          ...
          {showStickyCart && (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-base font-bold text-foreground">{currencySymbol}{price.toLocaleString("en-IN")}</p>
              </div>
              <button
                onClick={handleAddToCart}
