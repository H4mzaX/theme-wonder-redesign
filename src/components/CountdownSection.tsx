...
const CountdownSection = () => {
  ...
  return (
    <section className="relative mx-4 sm:mx-8 lg:mx-12 xl:mx-16 rounded-lg overflow-hidden my-20">
      <ParallaxImage src={countdownBg} alt="Sale" className="h-[400px] sm:h-[500px] rounded-lg" speed={0.15} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        ...
        <div className="flex gap-6 sm:gap-10 mb-8">
          {[
            ...
            <motion.div key={t.label} className="text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 * i, duration: 0.5 }}>
              <motion.span key={t.val} className="text-4xl sm:text-5xl font-display font-bold text-background block" initial={{ opacity: 0.7, y: -5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                {t.val}
              </motion.span>
        ...
        <motion.a href="#" className="bg-background text-foreground px-8 py-3 rounded-full text-sm font-medium" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
          Discover sales
        </motion.a>
      </div>
    </section>
