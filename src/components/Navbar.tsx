...
const Navbar = ({ onSearchOpen, onCartOpen, transparent = false }: NavbarProps) => {
  ...
  return (
    <>
      <motion.nav
        className={`section-padding w-full py-3.5 sm:py-4 fixed top-0 left-0 right-0 z-50 overflow-visible transition-all duration-300 will-change-transform ${
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                          <div className="pl-3 pb-3 flex flex-col gap-0.5">
                            {deviceSeries.map((group) => (
                              ...
                              >
                                <span className="text-[15px] font-medium text-foreground group-hover:text-accent transition-colors">{group.name}</span>
                              </Link>
                            ))}
                          </div>
                        </motion.div>
