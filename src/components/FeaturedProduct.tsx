          ...
          <ScrollReveal direction="right" delay={0.15} duration={0.8}>
            <div className="space-y-6">
              <div>
                ...
                <h3 className="text-3xl font-display font-semibold">{featuredProduct.name}</h3>
                <p className="text-xs text-muted-foreground">{featuredProduct.subtitle}</p>
                <p className="text-2xl font-display mt-2">{featuredProduct.price}</p>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {featuredProduct.colors.map((c, i) => (
                    <motion.button key={i} className={`w-8 h-8 rounded-full ${c.class} ${c.active ? "ring-2 ring-foreground ring-offset-2 ring-offset-section-alt" : ""}`} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }} />
                  ))}
                </div>
              </div>
