...
import { cardHover, tapScale, badgePop, premiumEase } from "@/lib/motion";
      ...
      >
        ...
        >
          ...
          {(() => {
            ...
            return (
              <motion.span
                className={`absolute top-2.5 left-2.5 z-10 ${style} text-[10px] sm:text-[11px] px-2.5 py-[3px] rounded font-semibold tracking-wide`}
                variants={badgePop}
                initial="hidden"
                animate="visible"
              >
          ...
          {/* Feature icons — mobile: 2, desktop: 3 */}
          <div
            className="absolute bottom-0 left-0 right-0 flex justify-center gap-1.5 sm:gap-2 px-2 sm:px-3 pb-2.5 sm:pb-3 pt-8 bg-gradient-to-t from-background/95 via-background/40 to-transparent sm:hidden"
            style={{
          ...
          >
            {features.slice(0, 2).map((feat, i) => (
              <div
                key={feat.label}
                className="flex items-center gap-1 bg-foreground/90 text-background rounded-full px-2.5 py-1.5 backdrop-blur-sm"
                style={{
                  opacity: isHovered ? 1 : 0,
        ...
        <div className="flex flex-col flex-1 px-4 pt-4 pb-4 gap-1">
          ...
          <motion.button
            ...
            className="w-full bg-foreground text-background text-[12px] sm:text-[13px] font-semibold py-3 rounded-lg tracking-wider mt-3"
